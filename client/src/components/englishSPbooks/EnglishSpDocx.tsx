...
const EnglishSpDocx = () => {
  const englishSPBooksRef = useRef<any>(null);
  const [searchParams, _] = useSearchParams();
  const navigate = useNavigate();
  const [fontSize, setFontSize] = useState(() => {
    const savedFontSize = localStorage.getItem("englishSpFontSize");
  ...
  useEffect(() => {
    localStorage.setItem("englishSpFontSize", fontSize.toString());
  }, [fontSize]);
  ...
  const {
    ...
    queryKey: ["englishSpBookdocxview", bookId],
    queryFn: () => {
  ...
  useEffect(() => {
    refetchBooks();
    englishSPBooksRef?.current?.scrollTo({ top: 0, behavior: "smooth" });
  }, [bookId]);
