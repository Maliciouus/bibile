...
const TamilSpecialbookList = () => {
  ...
  const {
    ...
    queryKey: ["tamil-specialbook"],
    queryFn: async ({ pageParam = 1 }: any) => {
      const response = await _axios.get(
        `/api/tamil-specialbook/getall?page=${pageParam}&limit=10`
      );
      ...
      {data?.pages?.map((page) =>
        page?.data?.map((book: any) => (
          <Card
            ...
            onClick={() => {
              navigate(
                `/specialbooks-tamil-layout?type=${book?.type}&BookId=${book?.id}`
              );
            }}
...
export default TamilSpecialbookList;
