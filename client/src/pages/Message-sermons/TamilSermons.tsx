...
const TamilSermons = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["getallsermons", pagination.page, pagination.limit],
    queryFn: () =>
      _axios.get(
        `/api/tamiltable/getall?page=${pagination.page}&limit=${pagination.limit}`
      ),
  });
  ...
  const handleDownload = (url: string) => {
    if (url) {
      // Regex to remove the hash (which could be alphanumeric) from the filename
      // const cleanedUrl = url.replace(/\/[a-zA-Z0-9]{10,}\./, "/"); // Adjust this if needed to match your hash pattern
      window.open(url, "_blank");
    }
  };
                          ...
                          {sermon?.audiopathurl ? (
                            <>
                              ...
                              <div className='flex items-center justify-center gap-5 '>
                                ...
                                <Icon
                                  onClick={
                                    () => handleDownload(sermon?.audiopathurl)
                                    // console.log(sermon?.audiopathurl?.url)
                                  }
                          ...
                          {sermon?.eng_to_tam_audiourl ? (
                            <>
                              ...
                              <div className='flex items-center justify-center gap-5 '>
                                ...
                                <Icon
                                  onClick={() =>
                                    handleDownload(sermon?.eng_to_tam_audiourl)
                                  }
                                  icon={"iconoir:download-square-solid"}
                        ...
                        <TableCell className='text-center py-2'>
                          {sermon?.tamilpdfurl ? (
                            <div className='flex items-center justify-center gap-5 '>
                              ...
                              <Icon
                                onClick={() => {
                                  handleDownload(sermon?.tamilpdfurl);
                                }}
                                icon={"iconoir:download-square-solid"}
                                className='h-7 w-7 font-bold text-[#EBB34A]  cursor-pointer '
                              />
