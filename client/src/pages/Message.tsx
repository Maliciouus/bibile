...
export default function RootMessage() {
  ...
  useEffect(() => {
    localStorage.setItem("messageFontSize", fontSize.toString());
  }, [fontSize]);

  const messageID =
    searchParams.get("m") || localStorage.getItem("MainMessageId") || 260;
  // @ts-ignore
  let storedMessageID = localStorage.setItem("MainMessageId", messageID);

  const { data, isLoading, refetch } = useQuery({
    queryKey: ["messages"],
    queryFn: () => _axios.get(`/api/message?m=${messageID}`),
    retry: false,
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    try {
      ...
      const response = await _axios.post(
        `/api/download/messagedownload?messageId=${messageID}&format=${data?.format}`,
        null,
        {
          headers: {
            "Content-Type": `${
      ...
      link.setAttribute(
        "download",
        `Message-${messageID} .${data?.format}` || "Download"
      );
    ...
    <>
      <main className=' '>
          ...
          <div>
            <div className='flex items-center justify-between'>
              <Sheet open={isSheetOpen} onOpenChange={setIsSheetOpen}>
                ...
                <SheetContent
                  ...
                  side={"left"}>
                  <SheetHeader>
                    {/* <Sidebar onMessageSelect={handleMessageSelect} /> */}
                    <MessageSideBar onMessageSelect={handleMessageSelect} />
                  </SheetHeader>
                </SheetContent>
              </Sheet>
            ...
            <div className='flex items-center gap-4 mt-2 py-2 bg-[#212121] rounded-md justify-around px-2'>
              <audio
                className='w-full'
                controls
                src={data?.data?.data?.[0]?.audiourl || ""}></audio>
        ...
        <div
          ...
          <div
            className=' font-nirmala   gap-y-2 px-2 md:px-4 py-5'
            ...
            {isLoading ? (
              ...
              <div className=' font-nirmala '>
                {renderParagraphs(data?.data?.data?.[0]?.message)}
              </div>
            )}
          </div>
        </div>
