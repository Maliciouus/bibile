...
const PubilshBookView = () => {
  const publishedBooksRef = useRef<any>(null);
  const [searchParams, _] = useSearchParams();
  const navigate = useNavigate();
  const [fontSize, setFontSize] = useState(() => {
    const savedFontSize = localStorage.getItem("bublishedFontSize");
  ...
  useEffect(() => {
    localStorage.setItem("bublishedFontSize", fontSize.toString());
  }, [fontSize]);
  ...
  const {
    ...
    queryFn: () => {
      return _axios.get(`/api/publishedbooks/getall?id=${bookId}`);
    },
  });
  useEffect(() => {
    refetchBooks();
    publishedBooksRef?.current?.scrollTo({ top: 0, behavior: "smooth" });
  }, [bookId]);
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    try {
      ...
      const response = await _axios.post(
        `/api/download/publishedbookdownload?bookId=${bookId}&format=${data?.format}`,
        null,
        {
          headers: {
            "Content-Type": `${
...
export default PubilshBookView;
