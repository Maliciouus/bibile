...
const EnglishMessageSermon = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () =>
      _axios.get(
        `/api/englishtable/getall?page=${pagination.page}&limit=${pagination.limit}&search=${searchQuery}`
      ),
  });
  ...
  const { mutate: uploadMutate, isPending } = useMutation({
    mutationFn: () => {
      return _axios.post(
        `/api/englishtable/upload`,
        {
          id: bookID,
          englishPdf: messagePdf,
          wrapper: WrapperPdf,
          englishAudio: audio,
        },
        {
  ...
  const { mutate: handleDelete } = useMutation<any>({
    mutationFn: (type: any) => {
      return _axios.delete(
        `/api/englishtable/delete?sermonId=${sermon.id}&type=${type}`
      );
    },
    onSuccess: (data: any) => {
      if (data?.status === 200) {
        ...
        <div className='flex gap-2 justify-between'>
          <div className='flex flex-col gap-2 items-start'>
            <h1 className='text-lg font-semibold md:text-2xl'>
              English Message Sermons
            </h1>
            <div className='text-xs font-500 text-slate-400'>
              Manage your English Message Sermons
            </div>
          </div>
          ...
          <Table className='w-full max-w-full border'>
            <TableHeader>
              <TableRow
                ...
                <TableHead className='text-center'>Eng- MP3</TableHead>
                  ...
                  <TableRow key={sermon?.id}>
                    ...
                    <TableCell className='text-center py-2'>
                      <Button
                        ...
                        disabled={!sermon?.englishPdfurl}
                        className={`text-center ${
                          sermon?.englishPdfurl ? "text-red-600" : ""
                        }`}
                        variant='outline'
                        size='icon'>
          ...
          <div className='grid gap-4 py-4'>
            <div className='grid grid-cols-1  items-center gap-4'>
              <div>
                ...
                {!sermon?.englishAudiourl || !sermon?.englishAudio ? (
                  <Input
                    id='audio'
                    type='file'
                    accept='audio/*'
                  ...
                  <div className=' '>
                    ...
                    <div
                      onClick={() => {
                        ...
                        setDeleteType("englishAudio");
                      }}
                      className='flex items-center justify-between'>
                      {sermon?.englishAudio?.split("/").pop()}
              ...
              <div>
                ...
                {!sermon?.englishPdfurl || !sermon?.englishPdf ? (
                  <Input
                    id='messagePdf'
                    type='file'
                  ...
                  <div className=' '>
                    ...
                    <div
                      onClick={() => {
                        setIsDeleteModalOpen(true);
                        setDeleteType("englishPdf");
                      }}
                      className='flex items-center justify-between'>
                      {sermon?.englishPdf?.split("/").pop()}
                      <div className='text-red-500 cursor-pointer text-xl'>
                        <Icon icon={"ion:trash-outline"} />
                      </div>
                    </div>
...
export default EnglishMessageSermon;
