...
const EditPublishedBooks = () => {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    try {
      ...
      const response = await _axios.put(
        `/api/publishedbooks/updatename?id=${selectedBookId}`,
        {
          name: data.name,
        }
      );
  ...
  const { mutate: uploadMutate, isPending } = useMutation({
    mutationFn: () => {
      return _axios.post(
        `/api/publishedbooks/uploadaudio?id=${audioId}`,
        {
          audio: audio,
        },
        {
  ...
  const { mutate: deleteMutate } = useMutation({
    mutationFn: (id: any) => {
      return _axios.delete(`/api/publishedbooks/deleteaudio?id=${id}`);
    },
    onSuccess: (data: any) => {
      if (data?.status === 200) {
  ...
  const {
    ...
    queryFn: () =>
      _axios.get(
        `/api/publishedbooks/getallpublishedbooks?page=${pagination.page}&limit=${pagination.limit}&search=${searchQuery}`
      ),
  });
                ...
                <TableRow key={book?.id}>
                  ...
                  <TableCell className="text-center py-0">
                    <Button
                      ...
                      disabled={
                        book?.audiopath === null && book?.audioname === null
                      }
                      className="text-center"
                      variant="outline"
                      size="icon"
                  ...
                  <TableCell className="text-center py-0">
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button
                          onClick={() => {
                            setSelectedBookname(book?.booktitle);
...
export default EditPublishedBooks;
