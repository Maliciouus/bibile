...
const PublishedBooks = () => {
  ...
  const { mutate, isPending } = useMutation<any>({
    ...
    mutationFn: () => {
      ...
      return _axios.post("/api/publishedbooks/create", formData, {
        headers: {
  ...
  const {
    ...
    queryKey: ["publishbooknames", searchQuery],
    queryFn: async ({ pageParam = 1 }: any) => {
      const response = await _axios.get(
        `/api/publishedbooks/getall?page=${pageParam}&limit=10&search=${searchQuery}`
      );
  ...
  const {
    ...
    queryFn: () => {
      return _axios.get(`/api/publishedbooks/getall?id=${currBook}`);
    },
    staleTime: 60000,
    enabled: !!currBook,
  });
  ...
  const { mutate: handleDelete } = useMutation({
    mutationFn: (id) => {
      return _axios.delete(`/api/publishedbooks/deleteBook?id=${id}`);
    },
    onSuccess: (data: any) => {
      if (data?.status === 200) {
  ...
  const { mutate: updatemutate, isPending: updatemutatepending } = useMutation({
    mutationFn: () => {
      return _axios.put(`/api/publishedbooks/updatecontent?id=${currBook}`, {
        content: bookContent,
      });
    },
    onSuccess: (data: any) => {
      ...
      <div className="flex gap-2 justify-between items-center">
        <div className="flex flex-col gap-2 items-start">
          <h1 className="text-lg font-semibold md:text-2xl">
            Published Books({data?.pages?.[0]?.totalCount?.total})
          </h1>
                    ...
                    {page?.data?.map((book: any, idx: any) => (
                      ...
                      >
                        <div
                          onClick={() => {
                            navigate(
                              `/layout/publishedbooks?bookid=${book?.id}&name=${book?.booktitle}`
                            );
                          }}
