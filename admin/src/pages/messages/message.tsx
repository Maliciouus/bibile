...
export default function Messages() {
  ...
  const {
    ...
    queryKey: ["booooks", searchQuery],
    queryFn: async ({ pageParam = 1 }: any) => {
      const response = await _axios.get(
        `/api/message?page=${pageParam}&limit=15&search=${searchQuery}`
      );
  ...
  const { mutate, isPending } = useMutation({
    ...
    mutationFn: () => {
      ...
      return _axios.post("/api/message/create", formData, {
        headers: {
  ...
  const { mutate: handleDelete } = useMutation({
    mutationFn: (id) => {
      return _axios.delete(`/api/message/delete?messageId=${id}`);
    },
    onSuccess: (data: any) => {
      if (data?.data?.status === 200) {
      ...
      <div className='flex gap-2 justify-between items-center'>
        <div className='flex flex-col gap-2 items-start'>
          <h1 className='text-lg font-semibold md:text-2xl'>
            Messages({data?.pages?.[0]?.totalCount?.count})
          </h1>
          <p className='text-xs font-500 text-slate-400'>
            Messages from Bible, and more
          </p>
        </div>
                    ...
                    {page?.data?.map((book: any, idx: any) => (
                      <div
                        ...
                        <div
                          onClick={() => {
                            navigate(
                              `/messagequil?m=${book?.messageId}&name=${book?.messageName}`
                            );
                          }}
                          className={`border-dashed w-[100%] p-2 border-slate-200 bg-slate-100 cursor-pointer hover:bg-slate-600 hover:text-white  rounded-m`}>
                          <h4>{book?.messageName}</h4>
                        </div>
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction
                                onClick={() => handleDelete(book?.messageId)}>
                                Continue
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
