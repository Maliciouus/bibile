  ...
  return (
    <main className='flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6'>
      <div className='flex flex-col gap-2 items-start'>
        <h1 className='text-lg font-semibold md:text-2xl'>Bible Edit </h1>
        <p className='text-xs font-500 text-slate-400'>All Chapters</p>
      </div>
        ...
        {data &&
          ...
          data.data.data.map((book: any, idx: any) => (
            <Card
              ...
              onClick={() => {
                navigate(
                  `/layout/_bible-edit/?type=bibleedit&book=${book.title}&chapterCount=${book.chapter_count}`
                );
              }}>
              <CardHeader>
                <Badge
