...
const EditTamilSpecialBooks = () => {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    try {
      ...
      const response = await _axios.put(
        `/api/tamil-specialbook/updatename?id=${selectedBookId}`,
        {
          name: data.name,
        }
      );
  ...
  const {
    ...
    queryFn: () =>
      _axios.get(
        `/api/tamil-specialbook/getalltamil-specialbooks?page=${pagination.page}&limit=${pagination.limit}&search=${searchQuery}`
      ),
  });
        ...
        <div className="flex gap-2 justify-between">
          <div className="flex flex-col gap-2 items-start">
            ...
            <div className="text-xs font-500 text-slate-400">
              Manage your Tamil special books
            </div>
          </div>
...
export default EditTamilSpecialBooks;
