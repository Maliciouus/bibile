...
const EnglishSpecialBooks = () => {
  ...
  const { mutate, isPending } = useMutation<any>({
    ...
    mutationFn: () => {
      ...
      return _axios.post("/api/english-specialbook/create", formData, {
        headers: {
  ...
  const {
    ...
    queryKey: ["english-specialbook", searchQuery],
    queryFn: async ({ pageParam = 1 }: any) => {
      const response = await _axios.get(
        `/api/english-specialbook/getall?page=${pageParam}&limit=10&search=${searchQuery}`
      );
  ...
  const {
    ...
    queryKey: ["english-specialbookss"],
    queryFn: () => {
      return _axios.get(`/api/english-specialbook/getall?id=${currBook}`);
    },
    staleTime: 60000,
    enabled: !!currBook,
  });
  ...
  const { mutate: handleDelete } = useMutation({
    mutationFn: (id) => {
      return _axios.delete(`/api/english-specialbook/deleteBook?id=${id}`);
    },
    onSuccess: (data: any) => {
      if (data?.status === 200) {
  ...
  const { mutate: updatemutate, isPending: updatemutatepending } = useMutation({
    mutationFn: () => {
      return _axios.put(
        `/api/english-specialbook/updatecontent?id=${currBook}`,
        {
          content: bookContent,
        }
      );
      ...
      <div className="flex gap-2 justify-between items-center">
        <div className="flex flex-col gap-2 items-start">
          <h1 className="text-lg font-semibold md:text-2xl">
            English Special Books({data?.pages?.[0]?.totalCount?.total})
          </h1>
                      ...
                      >
                        <div
                          onClick={() => {
                            if (book?.type === "docx") {
                              navigate(
                                `/layout/english?bookid=${book?.id}&name=${book?.title}`
                              );
                              console.log(book);
                            } else if (book?.type === "pdf") {
                              console.log(book);
                              navigate(
                                `/layout/english?bookid=${book?.id}&name=${book?.title}&pdf=true`
                              );
                              //   handlePDFView();
                            }
                          }}
...
export default EnglishSpecialBooks;
